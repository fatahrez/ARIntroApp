...
//  Matttress
//
//  Created by Abdulfatah Mohamed on 20/03/2021.
//
...
class ViewController: UIViewController, ARSCNViewDelegate, ARSessionDelegate, UIDocumentPickerDelegate {
    ...
    @IBOutlet weak var loadModelButton: FlashLightButton!
    ...
    var modelURL: URL? {
        didSet {
            ...
            if let detectedObject = self.testRun?.detectedObject {
                detectedObject.set3DModel(modelURL)
            }
        }
    }
    ...
    override func viewDidLoad() {
        ...
        notificationCenter.addObserver(self, selector: #selector(ghostBoundingBoxWasCreated), name: ScannedObject.ghostBoundingBoxCreatedNotification, object: nil)
        notificationCenter.addObserver(self, selector: #selector(ghostBoundingBoxWasRemoved), name: ScannedObject.ghostBoundingBoxRemovedNotification, object: nil)
        notificationCenter.addObserver(self, selector: #selector(boundingBoxWasCreated), name: ScannedObject.boundingBoxCreatedNotification, object: nil)
        notificationCenter.addObserver(self, selector: #selector(scanPercentageChanged), name: BoundingBox.scanPercentageChangedNotification, object: nil)
        notificationCenter.addObserver(self, selector: #selector(boundingBoxPositionOrExtentChanged(_:)), name: BoundingBox.extentChangedNotification, object: nil)
        notificationCenter.addObserver(self, selector: #selector(boundingBoxPositionOrextentChanged(_:)), name: BoundingBox.positionChangedNotification, object: nil)
        notificationCenter.addObserver(self, selector: #selector(objectOriginPositionChanged(_:)), name: ObjectOrigin.positionChangedNotification, object: nil)
        notificationCenter.addObserver(self, selector: #selector(displayWarningIfInLowPowerMode), name: Notification.Name.NSProcessInfoPowerStateDidChange, object: nil)
        
        setupnavigationBar()
        
        displayWarningInLowPowerMode()
    ...
    func backFromBackground() {
        if state == .scanning {
            ...
            let message = "The scan was interrupted. It is recommednded to restart the scan."
    ...
    @IBAction func loadModelButtonTapped(_ sender: Any) {
        ...
        let documentPicker = UIDocumentPickerViewController(documentTypes: ["ccom.pixar.universal-scene-description-mobile"], in: .import)
    ...
    @IBAction func toggleInstructionButtonTapped(_ sender: Any) {
    ...
    func createdAndShareReferenceObject() {
        guard let testRun = self.testRun, let object = testRun.refernceObject, let name = object.name else {
    ...
    func startLimitedTrackingTimer() {
        ...
        limitedTrackingTimer = Timer.scheduledTimer(withTimeInterval: 2.0, repeats: false) { _ in
            ...
            if scan.state == .defineBoundingBox || scan.state == .scanning || scan.state == .adjustingOrigin {
                ...
                let message = "Low tracking quality - it is unlikely that a good reference object can be generated frok this scan."
                let buttonLine = "Restart Scan"
                
                self.showAlert(title: title, message: message, buttonTitle: buttonLine, showCancel: true) { _ in
                    self.state = .startARSession
                }
            }
        }
    ...
    func startMaxScanTimeTimer() {
        ...
        let timeout: TimeInterval = 60.5 * 5
        ...
        maxScanTimeTimer = Timer.scheduledTimer(withTimeInterval: timeout, repeats: false) { _ in
            ...
            let message = "Scanning consumes a lot of resources. This scan has been running for \(Int(timeout)) s. Consider cloding the app and letting the device rest for a few minutes."
            let buttonTitle = "OK"
                ...
                if let scan = scan {
                    ...
                    case .defineBoundingBox, .scanning, .adjustingOrigin:
                        if reason == .relocalizing {
                            ...
                            let title = "Warning: Scan may be broken"
                            let message = "A gap in tracking has occured. It is recommended to restart the scan."
                            let buttonTitle = "Restart Scan"
                            self.showAlert(title: title, message: message, buttonTitle: buttonTitle, showCancel: true) {
                                self.state = .notReady
                            }
    ...
    func renderer(_ renderer: SCNSceneRnderer, updateAtTime time: TimeInterval) {
    ...
    fileprivate func mergeIntoCurrentScan(referenceObject: ARReferenceObject, from url: URL) {
        if self.state == .testing {
            ...
            // Try to merge the object which was just scanned with existing one.
            self.testRun?.referenceObject?.mergeInBackground(with: referenceObject, completion: { (mergedObject, error) in
                ...
                if let mergedObject = mergedObject {
                    ...
                    self.showAlert(title: "Merge successful", message: "The other scan has been erged into this scan.", buttonTitle: "OK", showCancel: false)
                } else {
    ...
    func scanPercentageChanged(_ notification: Notification) {
        ...
        DispatchQueue.main.async {
            self.setNavigateBarTitle("Scan (\(percentage)%")
        }
    }
    ...
    func displayWarningIfInLowPowerMode() {
        if ProcessInfo.processInfo.isLowPowerModeEnabled {
            ...
            let message = "Perfomance may be impacted. For best scanning results, disable Low Power Mode in Settings > Battery, and restart the scan."
            let buttonTitle = "OK"
